...
use crate::mm::{translated_refmut, translated_str, VirtAddr, PhysAddr, PageTable};
use crate::task::{
    add_task, current_task, current_user_token, exit_current_and_run_next,
    suspend_current_and_run_next, TaskStatus, malloc, mfree, get_task_info,
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    if let Some(app_data) = get_app_data_by_name(path.as_str()) {
        let new_task: Arc<TaskControlBlock> = Arc::new(TaskControlBlock::new(app_data));
